...
allProperties = [ "InvertModNaturalUnit", "InvertModWordUnit" ]
...
witnessesFor "InvertModNaturalUnit" =
  [ ("witness_invert_mod_natural_unit_case_zero",  W.witness_invert_mod_natural_unit_case_zero)
  , ("witness_invert_mod_natural_unit_case_seven", W.witness_invert_mod_natural_unit_case_seven)
  ]
witnessesFor "InvertModWordUnit" =
  [ ("witness_invert_mod_word_unit_case_zero",  W.witness_invert_mod_word_unit_case_zero)
  , ("witness_invert_mod_word_unit_case_seven", W.witness_invert_mod_word_unit_case_seven)
  ]
witnessesFor _ = []
...
runQuickCheck "InvertModNaturalUnit" =
  qcCheck (QC.forAll GQ.gen_invert_mod_natural_unit (toQc P.property_invert_mod_natural_unit))
runQuickCheck "InvertModWordUnit" =
  qcCheck (QC.forAll GQ.gen_invert_mod_word_unit (toQc P.property_invert_mod_word_unit))
runQuickCheck p = pure (Outcome "aborted" 0 Nothing (Just ("unknown property: " ++ p)))

toQc :: Show a => (a -> PropertyResult) -> a -> QC.Property
toQc f a = case f a of
  Pass     -> QC.property True
  Discard  -> QC.discard
  Fail msg -> QC.counterexample msg (QC.property False)

qcCheck :: QC.Property -> IO Outcome
qcCheck p = do
  r <- QC.quickCheckWithResult QC.stdArgs { QC.maxSuccess = 200, QC.chatty = False } p
  case r of
...
runHedgehog "InvertModNaturalUnit" =
  hhCheck (toHh GH.gen_invert_mod_natural_unit P.property_invert_mod_natural_unit)
runHedgehog "InvertModWordUnit" =
  hhCheck (toHh GH.gen_invert_mod_word_unit P.property_invert_mod_word_unit)
runHedgehog p = pure (Outcome "aborted" 0 Nothing (Just ("unknown property: " ++ p)))

toHh :: Show a => HH.Gen a -> (a -> PropertyResult) -> HH.Property
toHh g f = HH.property $ do
  a <- HH.forAll g
  case f a of
    Pass     -> pure ()
    Discard  -> HH.discard
...
hhCheck :: HH.Property -> IO Outcome
hhCheck p = do
  ok <- HH.check p
  if ok
    then pure (Outcome "passed" 100 Nothing Nothing)
...
runFalsify "InvertModNaturalUnit" =
  fsCheck GF.gen_invert_mod_natural_unit P.property_invert_mod_natural_unit
runFalsify "InvertModWordUnit" =
  fsCheck GF.gen_invert_mod_word_unit P.property_invert_mod_word_unit
runFalsify p = pure (Outcome "aborted" 0 Nothing (Just ("unknown property: " ++ p)))

fsCheck :: Show a => FG.Gen a -> (a -> PropertyResult) -> IO Outcome
fsCheck g f = do
  let prop = do
        a <- FP.gen g
        case f a of
          Pass     -> pure ()
          Discard  -> FP.discard
          Fail msg -> FP.testFailed (show a ++ ": " ++ msg)
  m <- FI.falsify prop
  case m of
...
runSmallCheck "InvertModNaturalUnit" =
  scCheck GS.series_invert_mod_natural_unit P.property_invert_mod_natural_unit
runSmallCheck "InvertModWordUnit" =
  scCheck GS.series_invert_mod_word_unit P.property_invert_mod_word_unit
runSmallCheck p = pure (Outcome "aborted" 0 Nothing (Just ("unknown property: " ++ p)))

scCheck :: Show a => SCS.Series IO a -> (a -> PropertyResult) -> IO Outcome
scCheck s f = do
  countRef <- newIORef (0 :: Int)
  let depth = 5
      check a = SC.monadic $ do
        modifyIORef' countRef (+1)
        pure $ case f a of
          Pass    -> True
          Discard -> True
          Fail _  -> False
      smTest = SC.over s check
